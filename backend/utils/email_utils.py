...
from googleapiclient.errors import HttpError # Added this import
    ...
    try:
        ...
        if error.resp.status == 403:
            logger.error(
                ...
                f"This likely means the application lacks the 'https://www.googleapis.com/auth/gmail.labels' "
                f"(or 'https://www.googleapis.com/auth/gmail.modify') scope. Error: {error}"
            )
            # You might want to raise a specific custom exception here to be caught by the caller
            # e.g., raise InsufficientPermissionsError(f"Missing gmail.labels or gmail.modify scope for user {user_id}")
